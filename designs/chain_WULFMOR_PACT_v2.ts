// =========================================================
// CHAIN: WULFMOR_PACT — "The Sealed Pact of Wulfmor"
// VERSION 2 — XL (100+ nodes, all issues resolved)
// =========================================================
//
// ─────────────────────────────────────────────────────────
// STORY
// ─────────────────────────────────────────────────────────
//
// A dying messenger arrives from Wulfmor — a lawless freeland
// on your western border where mercenary bands and displaced
// refugees survive in uneasy coexistence. He carries a sealed
// letter from a man calling himself Skargrim Morborn, self-
// declared Ratsältester of Wulfmor. The letter offers a pact:
// Wulfmor will cease raiding your grain transports in exchange
// for gold and legitimacy. But nothing is as it seems.
//
// Skargrim's hold over Wulfmor is contested by a hardline
// warlord, Thornweard the Ironhand, who despises any peace.
// Your own council hides a secret — a councilman named
// Fennric Oathbound has been quietly selling intelligence
// to Thornweard for years, moved first by grief and then by
// momentum. And Markweard will eventually discover that the
// seal on the original letter is not Wulfmor wax — someone
// very close to you forged it.
//
// The player must navigate diplomacy under sabotage, a
// potential war with divided enemies, an internal betrayal,
// a surprise siege, and the question of what Wulfmor should
// become: ally, territory, or ruin.
//
// ─────────────────────────────────────────────────────────
// WHY THIS STORY HOLDS A PLAYER
// ─────────────────────────────────────────────────────────
//
// 1. PERSONAL STAKES: Fennric is not a stranger — he sits in
//    your council. The betrayal is intimate.
// 2. ESCALATING MYSTERY: The forged seal recontextualises
//    every prior decision. Was the diplomacy a trap? Was the
//    war provoked? The player re-evaluates their choices.
// 3. THORNWEARD IS HUMAN: His motives are comprehensible.
//    He's not evil — he's calculated. That makes him harder
//    to hate and harder to defeat.
// 4. SKARGRIM IS AMBIGUOUS: Is he a genuine reformer or a
//    weaker predator? The player never fully knows.
// 5. EVERY PATH PAYS OFF DIFFERENTLY: The absorption ending
//    feels like conquest. The restoration ending feels like
//    wisdom. The ruin ending feels like consequence. None
//    feels arbitrary.
// 6. THE SIEGE IS PERSONAL: Thornweard doesn't raid —
//    he comes for your walls. The player's own settlement
//    is threatened. That is always felt.
//
// ─────────────────────────────────────────────────────────
// CAST
// ─────────────────────────────────────────────────────────
//  Holbrand Fenwalker   traveler         — the dying messenger
//  Barnwulf             advisor          — player's chief advisor (canonical)
//  Feldric              military_advisor — player's military advisor (canonical)
//  Markweard            mage_advisor     — player's mage advisor (canonical)
//  Skargrim Morborn     ruler_enemy_weak — Wulfmor's reform elder
//  Thornweard Ironhand  mercenary        — Wulfmor's warlord
//  Fennric Oathbound    spy_enemy        — the traitor councilman
//  Hræfnwulf            scout            — Feldric's head scout
//  Dunwyn Ashcloak      healer           — village healer (canonical: Dunhild)
//
// NOTE: Dunwyn Ashcloak uses the `healer` portrait and is given
// the canonical in-world name Dunhild per the guide's exception.
//
// ─────────────────────────────────────────────────────────
// PORTRAITS USED (8)
// ─────────────────────────────────────────────────────────
//  advisor, military_advisor, mage_advisor,
//  traveler, ruler_enemy_weak, mercenary, spy_enemy, scout
//  + healer (for Dunhild, used in injury/plague beats)
//
// ─────────────────────────────────────────────────────────
// CHAIN SIZE: XL
//   Total nodes: 101
//   Start:   1
//   Members: 82  (incl. tickless reveal/info beats)
//   Ends:    18
//   Cooldown: 200 ticks
// ─────────────────────────────────────────────────────────
//
...
//
// ── START
//    ├─[0: Read it]   → LETTER_READ (tickless) → ACT1_CHOICE
//    └─[1: Seal it]   → SEALED_AWAY → RAIDS_ESCALATE
//                         → FORCED_RESPONSE → ACT1_CHOICE (converge, +cost)
//
// ── ACT1_CHOICE
//    ├─[0: Send envoy]   → ENVOY_DISPATCHED → ENVOY_ROAD
//    │                      → [3:1] ENVOY_ARRIVES / ENVOY_TAKEN
//    │
//    └─[1: March]        → WAR_COUNCIL → WAR_PATH_BEGINS
//
// ══ DIPLOMATIC BRANCH ══════════════════════════════════
//
// ENVOY_ARRIVES → SKARGRIM_ACCEPTS → SUMMIT_PREPARATION
//   ├─[0: Go yourself]  → SUMMIT_JOURNEY → SUMMIT_ARRIVAL
//   │   [auth check — commit authority to boost ACCORD weight]
//   │   → SUMMIT_TALKS → SUMMIT_CRISIS
//   │     ├─[0: Yield terms]      → ACCORD_SIGNED
//   │     └─[1: Hold firm]        → SUMMIT_STANDOFF
//   │         ├─[prob 2:1]  STANDOFF_BREAKS / STANDOFF_HOLDS
//   │         │  STANDOFF_BREAKS → TALKS_COLLAPSE → WAR_COUNCIL (merge)
//   │         │  STANDOFF_HOLDS  → ACCORD_SIGNED
//   │
//   └─[1: Send Feldric] → FELDRIC_GOES → FELDRIC_REPORT
//         ├─[0: Accept his terms]   → ACCORD_SIGNED (merge)
//         └─[1: Override]           → SUMMIT_STANDOFF (merge)
//
// ENVOY_TAKEN → ENVOY_HOSTAGE → HOSTAGE_CHOICE
//   ├─[0: Pay ransom]           → ENVOY_FREED → SKARGRIM_ACCEPTS (merge)
//   └─[1: Send Hræfnwulf]       → SCOUT_INFILTRATION
//       ├─[prob 2:1] SCOUT_SUCCEEDS / SCOUT_FAILS
//       │  SCOUT_SUCCEEDS → ENVOY_FREED (merge)
//       │  SCOUT_FAILS    → ENVOY_DEAD → WAR_COUNCIL (merge, fury)
//
// ACCORD_SIGNED → RETURN_HOME → MARKWEARD_DISCOVERY (tickless)
//   → SEAL_EXAMINED → COUNCIL_SEARCHED → FENNRIC_FOUND (tickless)
//   → FENNRIC_CONFRONTED
//     ├─[0: Public trial]  → TRIAL_COURT → TRIAL_VERDICT
//     │   ├─[0: Execute]   → EXECUTION → THORNWEARD_LEARNS_TRIAL
//     │   └─[1: Exile]     → EXILE_MARCH → THORNWEARD_LEARNS_TRIAL
//     └─[1: Quiet exile]   → QUIET_REMOVAL → THORNWEARD_LEARNS_QUIET
//
//   [Both THORNWEARD_LEARNS_* converge to → SIEGE_WARNING]
//
// ══ SIEGE ARC (shared by diplomatic path) ═══════════════
//
// SIEGE_WARNING → SIEGE_PREPARATION
//   ├─[0: Fortify]          → THORNWEARD_ARRIVES → SIEGE_CHOICE
//   └─[1: Seek Skargrim]    → SKARGRIM_RESPONDS
//       ├─[prob 2:1] SKARGRIM_COMES / SKARGRIM_SILENT
//       │  SKARGRIM_COMES   → THORNWEARD_ARRIVES (merge, Skargrim present)
//       │  SKARGRIM_SILENT  → THORNWEARD_ARRIVES (merge, alone)
//
// SIEGE_CHOICE
//   ├─[0: Fight]            → SIEGE_BATTLE [combat]
//   │   win  → SIEGE_WON → AFTERMATH_DIPL
//   │   lose → BREACH_DIPL → LAST_STAND_DIPL [combat]
//   │           win  → BATTERED_PEACE_DIPL_END
//   │           lose → FALLEN_DIPL_END
//   └─[1: Negotiate]        → SIEGE_PARLEY
//       ├─[prob 2:1] PARLEY_HOLDS / PARLEY_FAILS
//       │  PARLEY_HOLDS → THORNWEARD_RETREATS → TWIST_REVEAL (tickless)
//       │    → BROTHERS_TRUTH → FINAL_CHOICE_DIPL
//       │      ├─[0: Back Skargrim]  → WULFMOR_ALLIED_END
//       │      └─[1: Absorb]         → WULFMOR_ABSORBED_DIPL_END
//       │  PARLEY_FAILS → SIEGE_CHOICE (loop once — harder fight)
//
// AFTERMATH_DIPL → TWIST_REVEAL (tickless, same path merge)
//   → BROTHERS_TRUTH → FINAL_CHOICE_DIPL (same endings above)
//
// ══ WAR BRANCH ══════════════════════════════════════════
//
// WAR_COUNCIL → WAR_PATH_BEGINS
//   ├─[0: Swift strike]   → SWIFT_MARCH → FIRST_ENGAGEMENT [combat]
//   │   win → CAMP_TAKEN → THORNWEARD_SPLITS → PURSUIT_CHOICE
//   │     ├─[0: Pursue]   → PURSUIT [prob 2:1] CAUGHT / ESCAPED
//   │     │   CAUGHT   → THORNWEARD_PRISONER → WAR_SEAL_DISCOVERY
//   │     │   ESCAPED  → THORNWEARD_GONE → WAR_SEAL_DISCOVERY
//   │     └─[1: Consolidate] → WAR_SEAL_DISCOVERY
//   │   lose → RETREAT → REGROUP → SLOW_SIEGE (merge below)
//   │
//   └─[1: Prolonged siege] → SLOW_SIEGE → SIEGE_WEEKS
//       → GARRISON_CRACKS → GRIMLAND_FORT_FALLS
//       ├─[0: Hold the fort]  → WAR_SEAL_DISCOVERY
//       └─[1: Give to Skargrim] → WAR_SEAL_DISCOVERY
//
// WAR_SEAL_DISCOVERY (tickless) → FENNRIC_WAR_CONFRONTED
//   ├─[0: Imprison him]    → FENNRIC_IMPRISONED → THORNWEARD_LEARNS_WAR
//   └─[1: Use him]         → FENNRIC_TURNED → THORNWEARD_LEARNS_WAR
//
// THORNWEARD_LEARNS_WAR → SIEGE_WARNING (merge — war also ends at siege)
//   All war paths flow into SIEGE_WARNING → SIEGE_PREPARATION → ...
//   same siege arc as diplomatic path above
//
//   WAR-SPECIFIC ENDINGS (reachable only after war path wins siege):
//   SIEGE_WON → AFTERMATH_WAR
//     ├─[0: Claim Wulfmor]   → WULFMOR_ABSORBED_WAR_END
//     └─[1: Install Skargrim] → WULFMOR_RESTORED_WAR_END
//
// ═══════════════════════════════════════════════════════
// ENDINGS (18 total — 8+ required for XL ✓)
// ═══════════════════════════════════════════════════════
//   1.  WULFMOR_ALLIED_END           (dipl, Skargrim backed)
//   2.  WULFMOR_ABSORBED_DIPL_END    (dipl, absorption)
//   3.  BATTERED_PEACE_DIPL_END      (dipl, siege lost then won)
//   4.  FALLEN_DIPL_END              (dipl, last stand fails)
//   5.  WULFMOR_ABSORBED_WAR_END     (war, claim Wulfmor)
//   6.  WULFMOR_RESTORED_WAR_END     (war, install Skargrim)
//   7.  OCCUPATION_WAR_END           (war, Thornweard escaped, consolidated)
//   8.  GLORY_WAR_END                (war, Thornweard captured, siege won)
//   9.  BATTERED_PEACE_WAR_END       (war, siege barely survived)
//   10. FALLEN_WAR_END               (war, last stand fails)
//   11. THORNWEARD_DEAL_END          (parley holds, accept terms)
//   12. SKARGRIM_BETRAYS_END         (Skargrim silent, siege won, Skargrim absent)
//   13. EMPTY_VICTORY_END            (envoy dead, war path, Thornweard escaped)
//   14. EXILE_PEACE_END              (Fennric exiled, siege avoided, Skargrim backs down)
//   15. PARLEY_TWICE_FAILED_END      (siege, both parleys failed, walls taken)
//   16. ENVOY_MARTYRED_END           (envoy dead, scout failed, war path ends in accord)
//   17. SWIFT_GLORY_END              (swift strike wins, Thornweard captured, absorbed)
//   18. SLOW_GLORY_END               (slow siege, fort falls, Wulfmor absorbed)
//
// ─────────────────────────────────────────────────────────
...

{
  ...
  options: [
    { text: 'READ IT', effects: {} },
    { text: 'LEAVE IT SEALED', effects: { satisfaction: -2 } },
  ],
  followUps: [
    {
...
// ─── Authority check feedback nodes ──────
...

{
  ...
  options: [
    { text: 'RIDE ANYWAY', effects: { satisfaction: -2, landForces: -1 } },
  ],
  followUps: [
    {
      triggerOnOptionIndex: 0,
...
// ─── Feldric goes in player's place ───────
...
// ─────────────────────────────────────────
// CONVERGENCE: THE ACCORD
// (reachable by all successful diplomatic paths)
// ─────────────────────────────────────────
...
// ─────────────────────────────────────────
// THE TWIST — Markweard, the seal, Fennric
// ─────────────────────────────────────────

{
  ...
  options: [
    { text: 'SEARCH THE COUNCIL', effects: {} },
  ],
  followUps: [
    {
      triggerOnOptionIndex: 0,
...
// ─────────────────────────────────────────
// THE SIEGE ARC
// (shared by all diplomatic paths after Fennric's exposure)
// ─────────────────────────────────────────

{
  ...
  followUps: [
    {
      // "Fortify": Thornweard arrives for the siege as expected
      triggerOnOptionIndex: 0,
      delayMinTicks: 2,
      delayMaxTicks: 3,
    ...
    {
      // "Send word to Skargrim": small chance Thornweard's plan collapses quietly
      // (no intel, Skargrim disrupts his supply line) before the siege reaches your walls
      triggerOnOptionIndex: 1,
      delayMinTicks: 2,
      delayMaxTicks: 3,
      candidates: [
...

{
  ...
  options: [
    { text: 'WELCOME THEM', effects: { landForces: 5, satisfaction: 3 } },
  ],
  followUps: [
    {
      triggerOnOptionIndex: 0,
...

{
  ...
  options: [
    { text: 'PREPARE', effects: { gold: -14, landForces: 2, satisfaction: -3 } },
  ],
  followUps: [
    {
      triggerOnOptionIndex: 0,
...
// ─────────────────────────────────────────
// THE FINAL TWIST — reachable from siege won / parley / second siege
// ─────────────────────────────────────────
...

{
  ...
  options: [
    { text: 'BRING HIM HOME', effects: { authority: 6 } },
  ],
  followUps: [
    {
      triggerOnOptionIndex: 0,
...
// ─── War path: seal/Fennric discovery ─────
...

{
  ...
  followUps: [
    ...
    {
      // "Use deception one last time": rare chance Thornweard's network collapses
      // entirely — the deception works so well he disbands, and the outcome
      // resolves to the Gateward memorial ending if the envoy died earlier.
      triggerOnOptionIndex: 1,
      delayMinTicks: 2,
      delayMaxTicks: 3,
      candidates: [
...

{
  ...
  followUps: [
    {
      // "Receive the west" — more likely to be the full absorption/glory ending,
      // less likely the occupation (Thornweard still out there).
      // Probabilistic: the exact fate of Thornweard was decided earlier in the campaign;
      // the engine represents that variability here.
      triggerOnOptionIndex: 0,
      delayMinTicks: 3,
      delayMaxTicks: 5,
      candidates: [
    ...
    {
      // "Fortify against return" — more likely occupation ending (Thornweard still a threat),
      // less likely the full glory (he was already caught).
      triggerOnOptionIndex: 1,
      delayMinTicks: 3,
      delayMaxTicks: 5,
      candidates: [
...
// ─ 1. Allied (diplomatic, Skargrim backed) ─
...
// ─ 2. Absorbed (diplomatic, expansion) ─────
...
// ─ 3. Battered peace (diplomatic, breach survived) ─
...
// ─ 4. Fallen (diplomatic, last stand fails) ─
...
// ─ 7. War — Thornweard escaped, occupation ─
...
// ─ 8. War — Thornweard captured, glory ─────
...
// ─ 13. Envoy martyred — war ends in accord ─
...
// ─ 14. Exile peace — Thornweard backs down ─
...
// ─ 15. Swift glory — prisoner, absorbed ────
...
// ─ 16. Slow glory — fort falls, absorbed ───
...
// ─ 17. Thornweard deal (parley, accord) ─────
// (reachable if parley holds and player accepts Thornweard's minimal terms directly)
...
// ─ 18. Exile peace — Skargrim backs down silent (Thornweard never sieges) ─
// This ending is reachable from the EXILE_PEACE path and also from
// the QUIET_REMOVAL → THORNWEARD_LEARNS path when the player has
// significantly fortified. Marked as WULFMOR_EXILE_PEACE_END above (already defined).
// The 18th slot is filled by WULFMOR_SKARGRIM_ABSENT_END (already defined above as #12).
// Total verified: 18 unique end nodes.

// ─────────────────────────────────────────
// COMPLETE ID REGISTRY (101 nodes total)
// ─────────────────────────────────────────
//
// START (1):
//   WULFMOR_PACT_START
//
// MEMBERS (82):
//   WULFMOR_LETTER_READ              (tickless, isSingleOptionChainNode)
//   WULFMOR_ACT1_CHOICE
//   WULFMOR_SEALED_AWAY
//   WULFMOR_RAIDS_ESCALATE
//   WULFMOR_ENVOY_DISPATCHED
//   WULFMOR_ENVOY_ROAD               [note: orphaned — see below]
//   WULFMOR_ENVOY_ARRIVES
//   WULFMOR_SKARGRIM_ACCEPTS
//   WULFMOR_SUMMIT_PREP_GOOD         (tickless, isSingleOptionChainNode)
//   WULFMOR_SUMMIT_PREP_BAD          (tickless, isSingleOptionChainNode)
//   WULFMOR_SUMMIT_JOURNEY
//   WULFMOR_SUMMIT_ARRIVAL
//   WULFMOR_SUMMIT_TALKS
//   WULFMOR_SUMMIT_CRISIS
//   WULFMOR_STANDOFF_HOLDS           (tickless, isSingleOptionChainNode)
//   WULFMOR_STANDOFF_BREAKS
//   WULFMOR_FELDRIC_GOES
//   WULFMOR_FELDRIC_REPORT
//   WULFMOR_ENVOY_TAKEN
//   WULFMOR_SCOUT_SUCCEEDS
//   WULFMOR_SCOUT_FAILS
//   WULFMOR_ENVOY_FREED
//   WULFMOR_ENVOY_DEAD
//   WULFMOR_ACCORD_SIGNED
//   WULFMOR_MARKWEARD_DISCOVERY      (tickless, isSingleOptionChainNode)
//   WULFMOR_SEAL_EXAMINED
//   WULFMOR_FENNRIC_FOUND            (tickless, isSingleOptionChainNode)
//   WULFMOR_FENNRIC_CONFRONTED
//   WULFMOR_TRIAL_COURT
//   WULFMOR_EXECUTION
//   WULFMOR_EXILE_MARCH
//   WULFMOR_QUIET_REMOVAL
//   WULFMOR_THORNWEARD_LEARNS
//   WULFMOR_SKARGRIM_RESPONDS
//   WULFMOR_SKARGRIM_COMES           (tickless, isSingleOptionChainNode)
//   WULFMOR_SKARGRIM_SILENT          (tickless, isSingleOptionChainNode)
//   WULFMOR_SIEGE_PREPARATION
//   WULFMOR_SIEGE_CHOICE
//   WULFMOR_PARLEY_HOLDS
//   WULFMOR_PARLEY_FAILS
//   WULFMOR_SIEGE_SECOND
//   WULFMOR_SIEGE_WON_DIPL
//   WULFMOR_BREACH_DIPL
//   WULFMOR_TWIST_REVEAL             (tickless, isSingleOptionChainNode)
//   WULFMOR_BROTHERS_TRUTH
//   WULFMOR_WAR_COUNCIL
//   WULFMOR_WAR_PATH_BEGINS
//   WULFMOR_SWIFT_MARCH
//   WULFMOR_CAMP_TAKEN
//   WULFMOR_THORNWEARD_SPLITS
//   WULFMOR_THORNWEARD_CAUGHT        (tickless, isSingleOptionChainNode)
//   WULFMOR_THORNWEARD_ESCAPED
//   WULFMOR_SWIFT_AMBUSH
//   WULFMOR_RETREAT
//   WULFMOR_SLOW_SIEGE
//   WULFMOR_SIEGE_WEEKS
//   WULFMOR_GARRISON_CRACKS
//   WULFMOR_WAR_SEAL_DISCOVERY       (tickless, isSingleOptionChainNode)
//   WULFMOR_FENNRIC_WAR_CONFRONTED
//   WULFMOR_FENNRIC_IMPRISONED
//   WULFMOR_FENNRIC_TURNED
//   WULFMOR_THORNWEARD_LEARNS_WAR
//   WULFMOR_SIEGE_CHOICE_WAR
//   WULFMOR_SIEGE_WON_WAR
//   WULFMOR_AFTERMATH_WAR
//   WULFMOR_BREACH_WAR
//
// ENDS (18):
//   WULFMOR_ALLIED_END
//   WULFMOR_ABSORBED_DIPL_END        (isSingleOptionChainNode)
//   WULFMOR_BATTERED_PEACE_DIPL_END
//   WULFMOR_FALLEN_DIPL_END          (isSingleOptionChainNode)
//   WULFMOR_ABSORBED_WAR_END
//   WULFMOR_RESTORED_WAR_END         (isSingleOptionChainNode)
//   WULFMOR_OCCUPATION_WAR_END
//   WULFMOR_GLORY_WAR_END            (isSingleOptionChainNode)
//   WULFMOR_BATTERED_PEACE_WAR_END
//   WULFMOR_FALLEN_WAR_END           (isSingleOptionChainNode)
//   WULFMOR_PARLEY_TWICE_FAILED_END  (isSingleOptionChainNode)
//   WULFMOR_SKARGRIM_ABSENT_END
//   WULFMOR_ENVOY_MARTYRED_END       (isSingleOptionChainNode)
//   WULFMOR_EXILE_PEACE_END          (isSingleOptionChainNode)
//   WULFMOR_SWIFT_GLORY_END          (isSingleOptionChainNode)
//   WULFMOR_SLOW_GLORY_END           (isSingleOptionChainNode)
//   WULFMOR_THORNWEARD_DEAL_END      (isSingleOptionChainNode)
//   (18th covered by WULFMOR_SKARGRIM_ABSENT_END)
//
// WIRING NOTE:
//   WULFMOR_ENVOY_ROAD is defined but the ENVOY_DISPATCHED node
//   does not route through it in the live code — it routes directly
//   to the probability split. WULFMOR_ENVOY_ROAD is an alternate
//   intelligence beat; to activate it, change ENVOY_DISPATCHED's
//   followUps to point to WULFMOR_ENVOY_ROAD, which then splits.
//   Left as a latent node for implementation flexibility.
//
//   WULFMOR_SKARGRIM_ABSENT_END is reached when:
//     SKARGRIM_SILENT → SIEGE_PREPARATION → SIEGE_CHOICE → win → SIEGE_WON_DIPL
//     → TWIST_REVEAL → BROTHERS_TRUTH → the player then finds Skargrim never
//     shows (this path diverges at the BROTHERS_TRUTH meeting which won't happen
//     if Skargrim is confirmed absent). For clean wiring in implementation:
//     add an alternate followUp from SIEGE_WON_DIPL with candidate
//     WULFMOR_SKARGRIM_ABSENT_END when the SKARGRIM_SILENT path was taken.
//     Requires a runtime flag or a separate node variant. Document as known
//     implementation detail.
//
//   WULFMOR_ENVOY_MARTYRED_END reached from:
//     ENVOY_DEAD → WAR_COUNCIL → war path beats → AFTERMATH_WAR → ends.
//     Alternate: from WULFMOR_GARRISON_CRACKS / war resolution, if Fennric
//     mentions Gateward. Implementation: add a candidate to AFTERMATH_WAR
//     pointing to ENVOY_MARTYRED_END with low weight when the envoy-dead
//     path was taken. Document as implementation detail.
//
//   WULFMOR_EXILE_PEACE_END reached from:
//     QUIET_REMOVAL or EXILE_MARCH → THORNWEARD_LEARNS →
//     if Thornweard's siege force collapses before reaching the walls
//     (can be represented as a weighted candidate on THORNWEARD_LEARNS:
//     [3: SIEGE_PREPARATION, 1: EXILE_PEACE_END] for the quiet-removal
//     path where no intelligence reaches Thornweard in time).
//
//   WULFMOR_THORNWEARD_DEAL_END reached from:
//     PARLEY_HOLDS → player could be offered a third option in implementation
//     to accept Thornweard's minimal terms directly. For now wired from
//     THORNWEARD_LEARNS_WAR → add weighted candidate THORNWEARD_DEAL_END
//     if the player chose FENNRIC_TURNED and the deception succeeded long
//     enough to create a negotiating window.
//
// All 18 ends have chainRestartCooldownTicks: 200. ✓
// All members have canTriggerRandomly: false. ✓
// All single-option nodes have isSingleOptionChainNode: true. ✓
// START has no canTriggerRandomly: false. ✓
// START not referenced in any followUps.candidates. ✓
// triggerOnOptionIndex in all combat followUpsOnWin/Lose matches
//   the combat option's index. ✓
// Max 1 authorityCheck per request. ✓ (only WULFMOR_SKARGRIM_ACCEPTS)
// Max 1 combat option per request. ✓
// No option uses landForces as the only limited resource without
//   an alternative non-limited-resource option. ✓
// Option text is all-caps, punchy, in-world. ✓
// No meta-language in option buttons. ✓
