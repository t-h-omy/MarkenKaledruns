/**
 * Building-based event modifiers
 *
 * These modifiers trigger when specific buildings are active and certain stat changes occur.
...
import { isBuildingActive } from './buildings';
 ...
 *
 * Only triggers if:
 * - Firewood building is active (at least one built)
 * - fireRisk delta > 0 (actual increase)
 */
export const firewoodModifier: ModifierHook = (state, _request, _optionIndex, baseDelta) => {
  const delta = { ...baseDelta };
  const extraChanges: AppliedChange[] = [];

  // Check if firewood building is active
  if (!isBuildingActive(state.buildingTracking, 'firewood')) {
 ...
 *
 * Only triggers if:
 * - Well building is active (at least one built)
 * - health delta > 0 (actual increase)
 */
export const wellModifier: ModifierHook = (state, _request, _optionIndex, baseDelta) => {
  const delta = { ...baseDelta };
  const extraChanges: AppliedChange[] = [];

  // Check if well building is active
  if (!isBuildingActive(state.buildingTracking, 'well')) {
    return { delta, extraChanges };
  }
...
/**
 * Array of all building modifiers that should be applied to event decisions.
